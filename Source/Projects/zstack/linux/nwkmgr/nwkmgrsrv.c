...
int appMain( apicHandle_t *handles )
{
  ...
  default_trace_enable_mask = 0xFF;
...
{ 
  ...
  {
    uiPrintf("%s:%d connection established\n", __FUNCTION__, __LINE__);
    // New Connection
    if( gNmNeedConfirm )
    {
  ...
  {
    uiPrintf("%s:%d connection terminated\n", __FUNCTION__, __LINE__);
    // Connection terminated
    return;
  }
  ...
  {
    uiPrintfEx(trDEBUG, "In sleep mode, App cmd %d ignored\n", cmdId );
    return;
  }

  uiPrintf("%s:%d new request sys=%x cmd %x, type %x\n", __FUNCTION__, __LINE__, subSys, cmdId, type);

  if ( (subSys & RPC_SUBSYSTEM_MASK) == ZSTACK_NWK_MGR_SYS_ID_T__RPC_SYS_PB_NWK_MGR )
  {
    // App to Network Manager
    nmHandleAppPbCb( connection, subSys, cmdId, len, pData, type );
  }
  else if ( (subSys & RPC_SUBSYSTEM_MASK) == ZSTACK_SERVER_SYS_ID_T__RPC_SYS_PB_SRVR )
  {
                      ...
                      uint8 *pData, uint8 type )
{
  ...
  uiPrintf("%s: cmd %d len %d\n", __FUNCTION__, cmdId, len);
  ...
  switch ( cmdId )
  {
    ...
    case NWK_MGR_CMD_ID_T__NWK_GET_DEVICE_LIST_REQ:
      ...
      {
        uiPrintf("%s: get dev list req NULL\n", __FUNCTION__);
      }
      break;
...
static void processNwkGetLocalDeviceInfoReq( int connection )
{
  ...
  if ( pBuf )
  {
    ...
    APIS_SendData( connection, ZSTACK_NWK_MGR_SYS_ID_T__RPC_SYS_PB_NWK_MGR, FALSE, 
...
static void processAfIncomingMsgInd( AfIncomingMsgInd *pInMsg )
{
  uiPrintf("AfIncomingMsgInd: addr 0x%04x, cmd %d, clusterID:0x%x, endpoint %d, rssi %d\n", pInMsg->srcaddr->shortaddr, pInMsg->cmdid, pInMsg->clusterid, pInMsg->endpoint, pInMsg->rssi );

  if (pInMsg->payload.len > 0)
  {
	  uiPrintfBuff(pInMsg->payload.len, pInMsg->payload.data);
  }
}
...
static void sendNwkZigbeeNwkReadyInd( NwkZigbeeNwkReadyInd *pNwkReadyInd )
{
  ...
  if ( pBuf )
  {
    ...
    APIS_SendData( -1, ZSTACK_NWK_MGR_SYS_ID_T__RPC_SYS_PB_NWK_MGR, TRUE, 
...
static void sendNwkGetNeighborTableRspInd( uint8 status, transTable_t *pTrans, ZdoMgmtLqiRspInd *pZdoMgmtLqiRspInd )
{
  ...
  if ( pBuf )
  {
    ...
    APIS_SendData( pTrans->connection, ZSTACK_NWK_MGR_SYS_ID_T__RPC_SYS_PB_NWK_MGR, TRUE, 
...
static void sendNwkSetBindingEntryRspInd( int connection, uint16 sequence, uint8 status, NwkAddressStructT *srcaddr )
{
  ...
  if ( pBuf )
  {
    ...
    // Send response back to app
    APIS_SendData( connection, ZSTACK_NWK_MGR_SYS_ID_T__RPC_SYS_PB_NWK_MGR, TRUE, 
