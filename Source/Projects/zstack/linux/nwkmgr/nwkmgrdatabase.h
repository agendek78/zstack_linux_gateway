...
#define gszNwkMgrDb_DeviceInfo_c     "DbDeviceInfo.bin"
#define gszNwkMgrDb_Endpoints_c      "DbEndpoints.csv"

// the database records get their ieeeAddr from the DeviceInfo
typedef struct __attribute__((__packed__)) sNwkMgrDb_Endpoint_tag
{
  uint8  endpointId;
  uint16 profileId;
  uint16 deviceId;
...
typedef struct __attribute__((__packed__)) sNwkMgrDb_DeviceInfo_tag
{
  uint64_t ieeeAddr;
  uint16   nwkAddr;
