...
{
  uiPrintfEx(trDEBUG, "znp_misc sendAPICZNP: subSys:%x, cmdID:%x, len:%d\n", subSys, cmdID, len );
                                     ...
                                     uint8 *pData )
{
  ...
  uiPrintfEx(trDEBUG, "znp_misc sendNPIExpectDefaultStatusZNP: subSys:%x, cmdID:%x, len:%d\n",
      subSys, cmdID, len );
  ...
  if ( subSys == MT_RPC_SYS_ZDO )
  {
    switch ( cmdID )
    {
      ...
      case MT_ZDO_END_DEVICE_ANNCE_IND:
          uiPrintfEx(trDEBUG, "MT_ZDO_END_DEVICE_ANNCE_IND\n");
        processZDODeviceAnnounceInd( len, pMsg );
        break;

      case MT_ZDO_TC_DEVICE_IND:
          uiPrintfEx(trDEBUG, "MT_ZDO_TC_DEVICE_IND\n");
        processZDOTCDeviceInd( len, pMsg );
        break;

      case MT_ZDO_PERMIT_JOIN_IND:
          uiPrintfEx(trDEBUG, "MT_ZDO_PERMIT_JOIN_IND\n");
  ...
  else if ( subSys == MT_RPC_SYS_SYS )
  {
    switch ( cmdID )
    {
      case MT_SYS_RESET_IND:
        {
          ...
          uiPrintfEx(trDEBUG, "znp_misc handleAsyncMsgs: Reset Indication\n" );
      ...
      case MT_SYS_JAMMER_IND:
          uiPrintfEx(trDEBUG, "MT_SYS_JAMMER_IND\n");
...
uint8 osal_nv_read( uint16 id, uint16 ndx, uint16 len, void *buf )
{
  ...
  uiPrintfEx(trDEBUG, "znp_misc osal_nv_read: id:%x, len:%d\n", id, len );
...
void znpReset( uint8 mode, uint8 nvReset, uint8 shutdown )
{
  ...
  uiPrintfEx(trDEBUG, "znp_misc znpReset: mode:%d, nvReset:%d, shutdown:%d\n", mode, nvReset, shutdown );
  ...
  uiPrintfEx(trDEBUG, "znp_misc znpReset: parameter:%d\n", parameter );
...
{
  uiPrintfEx(trDEBUG, "znp_misc znpSniffer: mode:%d\n", mode );
