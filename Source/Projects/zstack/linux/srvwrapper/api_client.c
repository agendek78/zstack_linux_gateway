                          ...
                          uint8 *pRxCmdId, uint16 *pRxLen )
{
  ...
  {
	uiPrintfBuff((int)(pInstance->numOfReceivedSRSPbytes - sizeof(apicMsgHdr_t)), ((const uint8_t *)(&(pInstance->srsp_msg[1]))));
    ...
    else if ( n == sizeof(hdrbuf) )
    {
      ...
      if ( pMsg )
      {
        ...
        if ( n == pMsg->len )
        {
          ...
          uiPrintfBuff(n, (const uint8_t *)&pMsg[1]);
          ...
          if ( (pMsg->subSys & RPC_CMD_TYPE_MASK) == RPC_CMD_SRSP )
          {
            ...
            uiPrintfEx(trINFO, "Client Read: (len %ld): \n", pInstance->numOfReceivedSRSPbytes );
            fflush( stdout );
