...
uint32_t default_trace_enable_mask = 0xFF & (~trMaskINFO); //DEFAULT_TRACE_ENABLE_MASK;
...
void printBuffer(uint32_t i_len, const uint8_t* i_pBuffer)
{
	int i = 0;
	...
	{
		int k;
		int num_to_print = i_len - i;

		for(k = 0; k < num_to_print && k < 16; k++)
		{
			if (k == 7)
			{
				printf("%02x_", i_pBuffer[i + k]);
			}
			else
			{
				printf("%02x ", i_pBuffer[i + k]);
			}
		}
		printf("\n");
		i += k;
