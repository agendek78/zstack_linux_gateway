...
void nwk_process_ready_ind(pkt_buf_t * pkt)
{
	...
	DBG_LOG("nwk_process_ready_ind: Received NWK_ZIGBEE_NWK_READY_IND");
	...
	{
		DBG_ERR("nwk_process_ready_ind: Error: Could not unpack msg");
	}
}
...
void nwk_process_info_cnf(pkt_buf_t * pkt, void * cbarg)
{
	...
	DBG_LOG("nwk_process_info_cnf: Received NWK_ZIGBEE_NWK_INFO_CNF");

	msg = nwk_zigbee_nwk_info_cnf__unpack(NULL, pkt->header.len, pkt->packed_protobuf_packet);

	if (msg)
	{
		DBG_LOG("msg->status = %d", msg->status);
		...
		if (msg->status == NWK_NETWORK_STATUS_T__NWK_UP)
		{
			...
			DBG_LOG("nwk_process_info_cnf: Network ready");
		}
		else
		{
	...
	{
		DBG_ERR("nwk_process_info_cnf: Error: Could not unpack msg");
	}
}
...
void nwk_send_info_request(void)
{
	...
	DBG_LOG("nwk_send_info_request: Sending NWK_INFO_REQ__INIT");
	...
	if (pkt)
	{
		...
		if (client_send_packet(pkt, (confirmation_processing_cb_t)&nwk_process_info_cnf, NULL) != 0)
		{
			DBG_ERR("nwk_send_info_request: Error: Could not send msg");
		}
	...
	{
		DBG_ERR("nwk_send_info_request: Error: Could not pack msg");
	}
}
